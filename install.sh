...
# Helper functions
# ---------------------------
require_root() { [[ "$EUID" -eq 0 ]] || { echo "ERROR: Must be run as root" >&2; exit 1; }; }
...
check_deps() {
  ...
  [[ ${#missing[@]} -eq 0 ]] || { echo "ERROR: Missing dependencies: ${missing[*]}" >&2; exit 1; }
}
...
check_tun() {
  ...
  [[ -c /dev/net/tun ]] || { echo "ERROR: TUN device unavailable" >&2; exit 1; }
}
...
main() {
  ...
  [[ -n "$TS_VER" ]] || { echo "ERROR: Could not detect latest Tailscale version" >&2; exit 1; }
  ...
  gpg --verify SHA256SUMS.sig SHA256SUMS >/dev/null 2>&1 || { echo "ERROR: Signature invalid" >&2; exit 1; }

  echo "Downloading Tailscale archive..."
  curl -fLO --connect-timeout 15 --max-time 600 "$URL"
  grep " $TGZ\$" SHA256SUMS | sha256sum -c - >/dev/null 2>&1 || { echo "ERROR: Checksum verification failed!" >&2; exit 1; }

  echo "Extracting..."
  tar --no-same-owner --no-same-permissions -xzf "$TGZ"
  EXTRACTED_DIR="$EXPECTED_DIR"

  [[ -x "$EXTRACTED_DIR/tailscale" && -x "$EXTRACTED_DIR/tailscaled" ]] || { echo "ERROR: binaries missing" >&2; exit 1; }
